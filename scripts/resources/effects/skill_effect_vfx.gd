...
# Cosmetic burst at the cast origin (host position — caster for at_caster skills,
# aim point for at_target, consistent with area_damage): explosion + hit sparks +
# optional screen flash + shake. Plays for both the local cast and the multiplayer
# visual-only copy (the old bespoke scripts did the same), so remote peers see it. A
# part is skipped when its driving value is zero (explosion_scale <= 0 / sparks_count
# == 0 / flash_time <= 0 / shake_time <= 0), so an effect can do just one of them.
...
func execute(_ctx: SkillContext, host: Node2D) -> void:
	if VfxManager == null:
		return
	var pos: Vector2 = host.global_position
	if explosion_scale > 0.0:
		VfxManager.spawn_explosion(pos, explosion_scale, explosion_color)
	if sparks_count > 0:
