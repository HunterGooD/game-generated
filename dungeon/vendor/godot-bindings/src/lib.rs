...
mod depend_on_prebuilt {
    ...
    pub fn write_gdextension_headers(h_path: &Path, rs_path: &Path, watch: &mut StopWatch) {
        ...
        // PATCH (cross-compile fix): upstream `load_gdextension_header_rs()` selects the
        // per-OS interface bindings via `#[cfg(target_os = ...)]`, which is evaluated for the
        // HOST the build script runs on — so cross-compiling from macOS always baked in the
        // macOS bindings (darwin pthread layout) and broke the Windows/Linux target build.
        // Dispatch on the actual target via CARGO_CFG_TARGET_OS instead, using the
        // (non-deprecated) target-aware API.
        let target_os = std::env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
        let platform = match target_os.as_str() {
            "windows" => prebuilt::TargetPlatform::Windows,
            "macos" | "ios" => prebuilt::TargetPlatform::MacOS,
            "emscripten" => prebuilt::TargetPlatform::Wasm,
            _ => prebuilt::TargetPlatform::Linux,
        };
